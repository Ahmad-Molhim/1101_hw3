...
task send_one_op;
  ...
  begin
    ...
    while (ready_pulse_cnt == old_ready_cnt)
      @(posedge clk);
...
initial begin
  ...
  for (i = 0; i < 64; i = i + 1) begin
    if (!is_x4(opmode_mem[i]))
      num_ops = num_ops + 1;
  end

  for (i = 0; i < 1024; i = i + 1) begin
    if (!is_x24(golden_mem[i]))
      num_golden = num_golden + 1;
  end
end
...
always @(posedge clk or negedge rst_n) begin
  if (!rst_n) begin
    ...
    if (o_op_ready)
      ready_pulse_cnt = ready_pulse_cnt + 1;

    if (o_out_valid) begin
      if (golden_idx >= num_golden) begin
        $display("[%0t] ERROR: Extra output after golden is exhausted. out_data=%h", $time, o_out_data);
        error_cnt = error_cnt + 1;
      end
      else if (o_out_data !== golden_mem[golden_idx]) begin
        $display("[%0t] ERROR: Output mismatch at golden[%0d]. got=%h expected=%h",
                 $time, golden_idx, o_out_data, golden_mem[golden_idx]);
        error_cnt = error_cnt + 1;
      end
...
initial begin
  ...
  if (ready_pulse_cnt !== num_ops) begin
    $display("ERROR: ready pulse count mismatch. got=%0d expected=%0d",
             ready_pulse_cnt, num_ops);
    error_cnt = error_cnt + 1;
  end

  if (golden_idx !== num_golden) begin
    $display("ERROR: golden count mismatch. got=%0d expected=%0d",
             golden_idx, num_golden);
    error_cnt = error_cnt + 1;
  end
