...
module ipdc (  //Don't modify interface
  ...
  reg finish_pending, prev_finish_pending;
  reg [23:0] o_out_data_w, o_out_data_r;
  reg [3:0] cur_op_mode, prev_op_mode;
  reg op_busy, prev_op_busy;
  ...
  reg [3:0] down_org_row, down_org_col;
  ...
  always @(*) begin
    ...
    o_out_data_w = 24'b0;
    o_out_valid_w = 1'b0;
    finish_pending = prev_finish_pending;
    ...
    fil_pix_col = 4'b0;
    fil_pixel = 24'b0;
    shifted_org_col = 4'b0;
    shifted_org_row = 4'b0;
    src_row = 4'b0;
    src_col = 4'b0;
    cur_op_mode = prev_op_mode;
    op_busy = prev_op_busy;
    if (i_op_valid && !prev_op_busy) begin
      cur_op_mode = i_op_mode;
      op_busy     = 1'b1;
    end
    if (prev_finish_pending) begin
      o_op_ready_w   = 1'b1;
      o_out_valid_w  = 1'b0;
      o_out_data_w   = 24'd0;
      finish_pending = 1'b0;
      ...
      if (prev_op_busy || (i_op_valid && !prev_op_busy)) begin
        case ((i_op_valid && !op_busy) ? i_op_mode : cur_op_mode)
          4'b0000: begin
            o_in_ready_w = 1'b1;  // ready while loading
            if (i_in_valid) begin
              Image[row_pos][col_pos] = i_in_data;
              if (prev_col_pos < 4'd15) begin
                col_pos = prev_col_pos + 4'd1;
              end else begin
                col_pos = 4'd0;
                if (prev_row_pos < 4'd15) begin
          ...
          4'b1100: begin
            ...
            else if (prev_image_size == 2'b01) disp_max = 1;  // 2x2 output
          ...
          4'b1101: begin
            if (prev_image_size == 2'b00) begin
              ...
              Cr_num = ($signed({1'b0, R}) << 2) -
                  (($signed({1'b0, G}) << 1) + $signed({1'b0, G})) - $signed({1'b0, B});
  ...
  always @(posedge i_clk or negedge i_rst_n) begin
    if (~i_rst_n) begin
      prev_row_pos <= 4'd00;
      prev_col_pos <= 4'd0;
      prev_org_row_pos <= 4'd0;
      prev_org_col_pos <= 4'd0;
      ...
      o_out_data_r <= 24'd0;
      o_out_valid_r <= 1'b0;
      prev_image_size <= 2'b0;
      prev_op_mode <= 4'b0;
      prev_op_busy <= 1'b0;
      prev_finish_pending = 1'b0;
      ...
      prev_org_col_pos <= org_col_pos;
      prev_image_size <= image_size;
      prev_op_mode <= cur_op_mode;
      prev_op_busy <= op_busy;
      prev_finish_pending <= finish_pending;
    end
  end
endmodule
